...
| # | 物件 | 規格 / 必須條件 | 約價 NT$ |
|---|---|---|---|
| 1 | **ESP32-S3-WROOM-1 N16R8 開發板** | **排針已焊好**、Native USB-C、PCB 天線 | 350–450 |
...
> **備註：實體材料行 vs 網購的價差** — 上面區間是台中／台北實體電子材料行的常見價格。蝦皮 / 露天 / Aliexpress 同一顆 S3-WROOM-1 N16R8 可能 NT$ 200–280，但**第一次跑硬體建議去實體店**，理由見下節「為什麼要到現場看（不能直接網購）」。
>
> **本專案的實際採購 (2026-04-26)**：在金華電子（光華商場）買到 ESP32-S3-WROOM-1 N16R8 焊好版 NT$ 400、杜邦線兩包，WS2812B Ring 改走蝦皮（要「預焊線版本」+ 單純電子材料行較難確認版本），詳見 [dev_setup.md](dev_setup.md)。
